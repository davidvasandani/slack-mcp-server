...
An array of [option objects](https://docs.slack.dev/reference/block-kit/composition-objects/option-object). Maximum number of options is 100. Each option must be less than 76 characters. If `option_groups` is specified, this field should not be.
...
An array of [option group objects](https://docs.slack.dev/reference/block-kit/composition-objects/option-group-object). Maximum number of option groups is 100. If `options` is specified, this field should not be.
...
An array of [option objects](https://docs.slack.dev/reference/block-kit/composition-objects/option-object) that exactly match one or more of the options within `options` or `option_groups`. These options will be selected when the menu initially loads.
...
A [confirm object](https://docs.slack.dev/reference/block-kit/composition-objects/confirmation-dialog-object) that defines an optional confirmation dialog that appears before the multi-select choices are submitted.
...
Indicates whether the element will be set to auto focus within the [`view object`](https://docs.slack.dev/reference/views). Only one element can be set to `true`. Defaults to `false`.
...
A [`plain_text` only text object](https://docs.slack.dev/reference/block-kit/composition-objects/text-object) that defines the placeholder text shown on the menu. Maximum length for the `text` in this field is 150 characters.
...
A [`plain_text`](https://docs.slack.dev/reference/block-kit/composition-objects/text-object) only text object that defines the placeholder text shown on the menu. Maximum length for the `text` in this field is 150 characters.
...
A [filter object](https://docs.slack.dev/reference/block-kit/composition-objects/conversation-filter-object) that reduces the list of available conversations using the specified criteria.
